...
Node* The_Problem::a_star_search(double (The_Problem::*heuristic)(const vector<vector<int>>& state) const){
    ...
    while (!frontier.empty()) {
        ...
        for (Node* child : children) {
            ...
            if (explored.find(child_string) == explored.end() && (frontier_map.find(child_string) == frontier_map.end() || frontier_map[child_string] > child->get_total_cost())) 
            { 
                int h = heuristic ? (this->*heuristic)(child->get_state()) : 0;
                child->set_heuristic(h);
...
Node* The_Problem::misplaced_tile_a_star() {
    return a_star_search(&The_Problem::heuristic_misplaced_tiles);
}

Node* The_Problem::euclidean_distance_a_star() {
    return a_star_search(&The_Problem::heuristic_euclidean_distance);
}
